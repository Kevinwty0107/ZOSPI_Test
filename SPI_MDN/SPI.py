...
#         A_list = [-max_action,-0.5*max_action,0,0.5*max_action,max_action]
#         #A_list = [-max_action,0,max_action]
#         num_choices = len(A_list)
#         n_dim = action_dim
#         self.act_array_fixed = np.zeros((num_choices**n_dim, n_dim))

#         for i in range(num_choices**n_dim):
#             result = tenToAny(num_choices,i)
#             while len(result)<n_dim:
#                 result.insert(0,'0')
#             for j in range(n_dim):
#                 self.act_array_fixed[i,j] = A_list[int(result[j])]
#             print(self.act_array_fixed[i])
#         print("finished action array_fixed initialization")
    ...
    def sample_action_and_select_max(self, state, action, mode = 'random', batch_size=64, sample_rep_num = 50, bcq_net = None, expl_noise = 0.5,phi=0.0):
        ...
        LG_indicator = np.zeros((batch_size,))
        ...
        if mode == 'random':
            ...
            for i in range(batch_size):
                ...
                mask[i] = 1.0 if value_baseline[i] < value[i][int(max_idx[i])] else 0.0
            #print('time used',dt.now()-start_time)
            return act_max_array, mask, None
        ...
        if mode == 'random_local':
            ...
            #print('act_array.shape 2',act_array.shape)
            if phi != 0.0:
            ...
            for i in range(batch_size):
                act_max_array[i] = act_array[i,int(max_idx[i]),:]
                mask[i] = 1.0 if value_baseline[i] < value[i][int(max_idx[i])] else 0.0
                #print('act_array.shape3',max_idx[i]*1.0/act_array.shape[1])
                
                LG_indicator[i] = 1.0 if max_idx[i]*1.0/act_array.shape[1] < 0.5 else 0.0 # LG = 1.0 ---> Global Exploitation
            #print('time used',dt.now()-start_time)
            return act_max_array, mask, LG_indicator
        
        
    def train_supervised_with_phi(self, replay_buffer, batch_size=100, sample_rep = 50, actor_training_time = 1, sample_mode = None, bcq=None):
        phi = self.phi
        self.total_it += 1
        self.counter_mask = []
        # Sample replay buffer 
        ...
        for i in range(actor_training_time):
            ...
            '''
            assert actor_training_time == 1, 'else need to revise the LG indi code below'
            action_alt, mask, LG_indi = self.sample_action_and_select_max(state,action,sample_mode, batch_size, sample_rep,bcq_net = bcq,phi = phi)
...
#             self.actor_optimizer.step()
        ...
        return self.counter_mask, mask, LG_indi
