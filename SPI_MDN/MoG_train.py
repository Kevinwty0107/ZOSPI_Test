...
def parse_args():
    ...
    parser.add_argument('-alg','--algorithm',type = str, default = SPI_MoG)
    parser.add_argument('-r','--repeat',type=int,default=None)
    parser.add_argument('-eps','--epsilon',type=float,default=0.0)
    parser.add_argument('-n','--nog',type=int,default=10)
    parser.add_argument('-ws','--warm_ups',type=int,default=25000)
    parser.add_argument('-bf','--buffer_size',type=int,default=1000000)
...
if __name__ == '__main__':
    ...
    alias = f'BS_{args.batch_size}_NoG_{args.nog}_EPS_{args.epsilon}_EXTAlias_{args.alias}'
    ...
    os.makedirs('MoGresults',exist_ok=True)
    os.makedirs('MoGckpts',exist_ok=True)
    os.makedirs('MoGlogs',exist_ok=True)
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    args_eval_freq = 1000
    args_start_timesteps = args.warm_ups
    ...
    for repeat in range(args.repeat, args.repeat + 1):
        if True:
            ...
            if args_policy == "TD3":
                ...
                if args.algorithm == 'SPI_MoG':
                    policy = SPI_MoG.SPI(**kwargs)
                else:
                    print('Error in Declaring Algo.')
        ...
        for t in range(int(args_max_timesteps)):
            ...
            if t >= args_start_timesteps:
                ...
                for i in range(1):
                    policy.train_supervised_with_phi(replay_buffer, args_batch_size, sample_mode ='random_local')
                    '''Supervised Learning'''
            ...
            if (t + 1) % args_eval_freq == 0:
                ...
                policy.save('MoGckpts/{}_{}_mog_epoch{}'.format(alias,ENV_NAME,t+1))
                np.save('MoGresults/evaluations_alias{}_ENV{}_Repeat{}'.format(alias,ENV_NAME,repeat),evaluations)